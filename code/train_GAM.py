...
parser.add_argument("--GLC_loss", type=float, default=0.005)
...
parser.add_argument("--output_dir", type=str, default="Group_style_aware")
...
with open("record_gt_a.json", 'r') as load_f:
    load_dict_a = json.load(load_f)

with open("record_gt_b.json", 'r') as load_f:
    load_dict_b = json.load(load_f)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            ...
            loss_a = 0
            loss_b = 0
            for temp in range(fake_B.shape[0]):
                split_name = img_name[temp].split('_')
                if len(split_name) != 2:
                    img_name[temp] = split_name[0] + "_" + split_name[1] + ".tif"
                fake_B_i = fake_B[temp, :, :, :].unsqueeze(0)
                img_a, img_b = calculate_Lab_ab(fake_B_i)
                a = torch.from_numpy(np.array(load_dict_a[img_name[temp]])).cuda()
                b = torch.from_numpy(np.array(load_dict_b[img_name[temp]])).cuda()

                loss_a += torch.var(torch.cat([a, img_a], dim=0))
                loss_b += torch.var(torch.cat([b, img_b], dim=0))
            ...
            loss = mse + \
                   opt.lambda_smooth * (weights_norm + tv_cons) + \
                   opt.lambda_monotonicity * mn_cons + \
                   opt.GLC_loss * (loss_a + loss_b)
    ...
    if epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(Context.state_dict(), "saved_models/%s/Context_%d.pth" % (opt.output_dir, epoch))
        torch.save(MaskContext.state_dict(), "saved_models/%s/MaskContext_%d.pth" % (opt.output_dir, epoch))
